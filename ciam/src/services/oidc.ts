...
export async function registerOidcRoutes(app: FastifyInstance) {
  ...
  app.route({
    ...
    handler: async (req: any, reply) => {
      ...
      const {
        response_type, client_id = "public-client", redirect_uri, scope,
        code_challenge, code_challenge_method = "S256", state,
        user_id, username, acr = "urn:webauthn:uv",
      } = q;
      if (response_type !== "code") throw createError("INVALID_REQUEST", 400, "unsupported_response_type");
      if (!user_id && !username) throw createError("INVALID_REQUEST", 400, "login_required");
  ...
  app.post("/oauth/token", async (req: any, reply) => {
    const { grant_type } = req.body || {};
    if (grant_type === "authorization_code") {
      const { code, redirect_uri, code_verifier } = req.body;
      const rec = AuthStore.consumeCode(code);
      if (!rec) throw createError("INVALID_GRANT", 400, "invalid_grant");
      ...
      if (actual !== rec.code_challenge) throw createError("INVALID_GRANT", 400, "pkce_failed");
      // Optional DPoP binding at token issuance
      let cnf: any = undefined;
      if (req.body?.dpop_proof) {
        ...
        try {
          ...
          cnf = { jkt: res.jkt };
        } catch (e: any) { throw createError("INVALID_DPOP", 400, e?.message || "dpop_failed"); }
      }
    ...
    if (grant_type === "refresh_token") {
      ...
      if (!r) throw createError("INVALID_GRANT", 400, "invalid_grant");
    ...
    if (grant_type === "ciba" || grant_type === "urn:openid:params:grant-type:ciba") {
      ...
      if (!s) throw createError("INVALID_REQUEST", 400, "invalid_request");
      if (s.status === "pending") {
        const now = Date.now();
      ...
      if (s.status === "denied") { audit.log("CIBA_DENIED", { auth_req_id }, req); throw createError("access_denied", 400, "access denied"); }
      if (s.status === "expired") { audit.log("CIBA_EXPIRED", { auth_req_id }, req); throw createError("expired_token", 400, "expired"); }
    ...
    if (grant_type === "webauthn") {
      ...
      if (!user_id || !response) throw createError("INVALID_REQUEST", 400, "missing_webauthn_params");
      const user = MemoryStore.getUser(user_id);
      const chall = MemoryStore.getChallenge(user_id);
      if (!user || !chall || chall.kind !== "authentication") throw createError("INVALID_REQUEST", 400, "challenge_missing_or_user_not_found");
      ...
      if (!verification.verified || !verification.authenticationInfo) throw createError("INVALID_REQUEST", 400, "webauthn_verify_failed");
