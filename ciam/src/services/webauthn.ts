...
import type { UserRecord, StoredCredential, ChallengeRecord } from "../types.js";
...
function pickField(obj: AnyRecord | undefined, keys: string[]): string | undefined {
...
function toBuf(b64url: string): Buffer {
...
async function buildRegistrationOptionsPayload(args: {
  ...
  let user: UserRecord | undefined = MemoryStore.getUser(userId);
  if (!user) {
  ...
  const enriched = { ...options } as AnyRecord;
  enriched.chal_id = challengeRec.chalId;
  return enriched;
}

async function buildAuthenticationOptionsPayload(user: UserRecord) {
  const roamingHint: AuthenticatorTransportFuture[] | undefined = config.webauthnRoamingOnly ? ["usb", "ble", "nfc"] : undefined;
...
function resolveChallenge(kind: ChallengeRecord["kind"], userId: string, body: AnyRecord) {
  const chalId = pickField(body, CHALLENGE_ID_KEYS);
  const challengeVal = pickField(body, CHALLENGE_VALUE_KEYS);
  let chall: ChallengeRecord | undefined;
  if (chalId) chall = MemoryStore.getChallengeById(chalId);
  if (!chall) chall = MemoryStore.getChallenge(userId);
  if (!chall && challengeVal) chall = MemoryStore.findChallengeByValue(challengeVal);
...
export async function registerWebAuthnRoutes(app: FastifyInstance) {
  ...
  }>("/webauthn/registration/options", async (req, reply) => {
  ...
  app.post<{
    Body: {
      userId?: string;
      user_id?: string;
      response?: any; // PublicKeyCredential from browser
      assertion?: any;
      chal_id?: string;
      challenge_id?: string;
      challenge?: string;
    };
  }>("/webauthn/registration/verify", async (req, reply) => {
    const userId = pickField(req.body, USER_ID_KEYS);
    if (!userId) {
      throw createError("INVALID_REQUEST", 400, "missing_user_id");
    }
    const bodyResp = (req.body as AnyRecord).response ?? (req.body as AnyRecord).assertion;
    if (!bodyResp) {
    ...
    const chall = resolveChallenge("registration", user.id, req.body as AnyRecord);
    ...
    try {
      ...
      const transports = bodyResp?.response?.transports as StoredCredential["transports"];
      ...
      MemoryStore.upsertUser(user);
      if (chall.chalId) MemoryStore.clearChallengeById(chall.chalId);
  ...
  app.post<{
    Body: { userId?: string; user_id?: string; username?: string; user_name?: string };
  }>("/webauthn/authentication/options", async (req, reply) => {
    let user: UserRecord | undefined;
    const userId = pickField(req.body, USER_ID_KEYS);
    if (userId) user = MemoryStore.getUser(userId);
    if (!user) {
      const uname = pickField(req.body, USERNAME_KEYS);
      if (uname) user = MemoryStore.getUserByUsername(uname);
    }
    if (!user) throw createError("INVALID_REQUEST", 400, "user_not_found");
  ...
  app.post<{
  ...
  }>("/webauthn/authentication/verify", async (req, reply) => {
    const userId = pickField(req.body, USER_ID_KEYS);
    if (!userId) {
      throw createError("INVALID_REQUEST", 400, "missing_user_id");
    }
    ...
    const user = MemoryStore.getUser(userId);
    if (!user) throw createError("INVALID_REQUEST", 400, "user_not_found");
    const chall = resolveChallenge("authentication", user.id, req.body as AnyRecord);
