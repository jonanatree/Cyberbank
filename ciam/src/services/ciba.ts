...
export async function registerCibaRoutes(app: FastifyInstance) {
  // Spec-alias: backchannel authentication start (alias of /ciba/auth)
  app.post("/backchannel/authentication", async (req: any, reply) => {
    const { client_id = "web", scope = "openid", login_hint, binding_message } = req.body || {};
    if (!login_hint) throw createError("INVALID_REQUEST", 400, "login_hint required");
    const now = Date.now();
  ...
  app.post("/ciba/auth", async (req: any, reply) => {
    const { login_hint, user_id, client_id = "public-client", scope, level = "L2" } = req.body || {};
    const uid = user_id || login_hint;
    if (!uid) throw createError("INVALID_REQUEST", 400, "login_hint_required");
    const now = Date.now();
    const sess = AuthStore.createCibaSession({
      client_id,
      user_id: uid,
  ...
  app.get("/ciba/approve/:id", async (req: any, reply) => {
    const id = req.params.id as string;
    const s = AuthStore.getCiba(id);
    if (!s) throw createError("INVALID_REQUEST", 400, "not_found");
    const action = (req.query?.action || "").toString();
    if (action === "approve") AuthStore.setCibaStatus(id, "approved");
    if (action === "deny") AuthStore.setCibaStatus(id, "denied");
    const status = AuthStore.getCiba(id)?.status ?? s.status;
    // notify subscribers of status change
    Push.notify(s.user_id, { type: "ciba_update", auth_req_id: id, status });
    if (status === "approved") audit.log("CIBA_APPROVED", { auth_req_id: id, user_id: s.user_id }, req);
    if (status === "denied") audit.log("CIBA_DENIED", { auth_req_id: id, user_id: s.user_id }, req);
  ...
  app.post("/ciba/approve/:id", async (req: any, reply) => {
    const id = req.params.id as string;
    const s = AuthStore.getCiba(id);
    if (!s) throw createError("INVALID_REQUEST", 400, "not_found");
    ...
    Push.notify(s.user_id, { type: "ciba_update", auth_req_id: id, status });
    if (approve) audit.log("CIBA_APPROVED", { auth_req_id: id, user_id: s.user_id }, req);
    if (!approve) audit.log("CIBA_DENIED", { auth_req_id: id, user_id: s.user_id }, req);
    return reply.send({ status: status.toUpperCase() });
  });

  // Fallback JSON approval without params (for testing environments)
  app.post("/ciba/approve-json", async (req: any, reply) => {
    ...
    const s = AuthStore.getCiba(id);
    if (!s) throw createError("INVALID_REQUEST", 400, "not_found");
    if (!user_id || user_id !== s.user_id) { audit.log("CIBA_DENIED", { auth_req_id: id, reason: "user_mismatch" }, req); throw createError("access_denied", 400, "access denied"); }
    const current = AuthStore.getCiba(id)?.status ?? s.status;
    if (current !== "pending") { return reply.send({ status: current.toUpperCase(), ...(current === "approved" && requireL3 ? { acr: "urn:step-up:L3" } : {}) }); }
    AuthStore.setCibaStatus(id, approve ? "approved" : "denied");
    const status = AuthStore.getCiba(id)?.status ?? s.status;
    if (approve && requireL3) { Push.notify(s.user_id, { type: "ciba_update", auth_req_id: id, status, acr: "urn:step-up:L3" }); audit.log("CIBA_APPROVED", { auth_req_id: id, user_id: s.user_id, acr: "urn:step-up:L3" }, req); return reply.send({ status: status.toUpperCase(), acr: "urn:step-up:L3" }); }
    Push.notify(s.user_id, { type: "ciba_update", auth_req_id: id, status }); if (approve) audit.log("CIBA_APPROVED", { auth_req_id: id, user_id: s.user_id }, req); else audit.log("CIBA_DENIED", { auth_req_id: id, user_id: s.user_id }, req);
    return reply.send({ status: status.toUpperCase() });
  });

  // SSE stream for mobile authenticators to receive push requests
  app.get("/ciba/stream", async (req: any, reply) => {
    const user_id = (req.query?.user_id || "").toString();
    if (!user_id) throw createError("INVALID_REQUEST", 400, "user_id_required");
    Push.add(user_id, reply);
  });

  // SSE path param variant: /ciba/stream/:userId
  app.get("/ciba/stream/:userId", async (req: any, reply) => {
  ...
  app.get("/ciba/_dump", async () => {
    try {
      // @ts-ignore
      const list = (AuthStore as any).listCiba ? (AuthStore as any).listCiba() : [];
  ...
  app.get("/ciba/mobile", async (req: any, reply) => {
    const user_id = (req.query?.user_id || "").toString();
