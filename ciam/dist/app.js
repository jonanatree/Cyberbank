...
export function buildApp() {
    ...
    if (config.showDemos.includes("demo"))
        app.get("/demo", async (_req, reply) => { reply.type("text/html").send(DEMO_HTML); });
    if (config.showDemos.includes("demo-l3"))
        app.get("/demo-l3", async (_req, reply) => { reply.type("text/html").send(DEMO_L3_HTML); });
    if (config.showDemos.includes("demo-web"))
        app.get("/demo-web", async (_req, reply) => { reply.type("text/html").send(DEMO_WEB_HTML); });
    if (config.showDemos.includes("demo-approver"))
        app.get("/demo-approver", async (_req, reply) => { reply.type("text/html").send(DEMO_APPROVER_HTML); });
    if (config.showDemos.includes("demo-passkey"))
        app.get("/demo-passkey", async (_req, reply) => { reply.type("text/html").send(DEMO_PASSKEY_HTML); });
    if (config.showDemos.includes("demo-tx"))
        app.get("/demo-tx", async (_req, reply) => { reply.type("text/html").send(DEMO_TX_HTML); });
    if (config.showDemos.includes("demo-dpop"))
        app.get("/demo-dpop", async (_req, reply) => { reply.type("text/html").send(DEMO_DPOP_HTML); });
    registerWebAuthnRoutes(app);
    registerOidcRoutes(app);
    registerPolicyRoutes(app);
    registerCibaRoutes(app);
    if (config.enableTxnSigning)
        registerTxnRoutes(app);
    ...
    app.addHook("onReady", async () => {
        ...
        setInterval(() => {
            try {
                AuthStore.sweepExpiredCiba((s) => audit.log("CIBA_EXPIRED", { auth_req_id: s.id, user_id: s.user_id }));
            }
            catch { }
            try {
                AuthStore.sweepJti();
...
async function reg(){
  ...
  }catch(e){ out('register error', e?.message||e); }
}
...
async function authn(){
  ...
  }catch(e){ out('authenticate error', e?.message||e); }
}
