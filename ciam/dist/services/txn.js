...
const CHALS = new Map();
export async function registerTxnRoutes(app) {
    app.log.info("registerTxnRoutes enabled");
    // New M0 flow: WebAuthn-based dynamic linking challenge
    app.post("/txn/challenge/webauthn", async (req, reply) => {
        const auth = req.headers["authorization"];
        if (!auth?.startsWith("Bearer "))
            throw createError("UNAUTHENTICATED", 401, "unauthenticated");
        if (!config.jwtPrivateKeyPEM)
            throw createError("INTERNAL", 500, "server_key_unset");
        // Identify user from AT (demo内在同进程验证)
        let at;
        try {
            const { payload } = await jwtVerify(auth.slice(7), await importPKCS8(config.jwtPrivateKeyPEM, "PS256"));
            at = payload;
        }
        catch {
        ...
        if (!txnId || amount == null || !currency || !payee)
            throw createError("invalid_txn", 400, "invalid_txn");
        ...
        const rec = { id: chal_id, userId, tbs, tbsHash, createdAt: Date.now(), expiresAt: Date.now() + TX_TTL * 1000, route };
        ...
        const webauthnOptions = { challenge: Buffer.from(tbsHash, "base64url"), rpId: config.rpId, userVerification: "required", allowCredentials, timeout: TX_TTL * 1000 };
    ...
    app.post("/txn/confirm", async (req) => {
        const { chal_id, method, assertion } = req.body || {};
        if (!chal_id || method !== "webauthn")
            throw createError("INVALID_REQUEST", 400, "invalid_request");
        const rec = CHALS.get(chal_id);
        if (!rec)
            throw createError("challenge_expired", 400, "challenge_expired");
        if (rec.signed)
            throw createError("already_signed", 400, "already_signed");
        if (Date.now() > rec.expiresAt) {
            CHALS.delete(chal_id);
            throw createError("challenge_expired", 400, "challenge_expired");
        }
        ...
        const jti = newJti();
        const payload = { sub: rec.userId, txn: rec.tbs, txn_hash: rec.tbsHash, method: "webauthn", jti, aud: "api", ...(rec.route ? { route: rec.route } : {}) };
    ...
    app.post("/txn/challenge", async (req, reply) => {
        ...
        const pk = await importPKCS8(config.jwtPrivateKeyPEM, "PS256");
        const jti = newJti();
        const payload = { txnId, amount, currency, payee, payer, purpose, hash, iat: nowSec(), exp: nowSec() + TX_TTL, jti };
    ...
    app.post("/txn/verify", async (req, reply) => {
        ...
        try {
            ...
            const { payload } = await jwtVerify(token, await importPKCS8(config.jwtPrivateKeyPEM, "PS256"));
            const p = payload;
            if (p.exp && nowSec() > p.exp) {
                audit.log("TX_INVALID", { reason: "expired", jti: p.jti, txnId: p.txnId }, req);
                throw createError("TXSIGN_INVALID", 403, "expired");
            }
            if (AuthStore.isJtiUsed(p.jti)) {
                audit.log("TX_REPLAY_BLOCKED", { jti: p.jti, txnId: p.txnId }, req);
                throw createError("REPLAY_DETECTED", 429, "replay");
            }
            if (tx_sig && tbs) {
                const t = canonicalTbs(tbs);
                const h = sha256b64url(serializeTbs(t));
                if (p.txn_hash !== h) {
                    audit.log("TXN_VERIFY_FAIL", { reason: "mismatch" }, req);
                    throw createError("TXSIGN_INVALID", 403, "mismatch");
                }
            }
            else {
                if (txnId && p.txnId !== txnId) {
                    audit.log("TX_INVALID", { reason: "txnId_mismatch", jti: p.jti }, req);
                    throw createError("TXSIGN_INVALID", 403, "txnId_mismatch");
                }
                if (amount != null && p.amount !== amount) {
                    audit.log("TX_INVALID", { reason: "amount_mismatch", jti: p.jti }, req);
                    throw createError("TXSIGN_INVALID", 403, "amount_mismatch");
                }
                if (currency && p.currency !== currency) {
                    audit.log("TX_INVALID", { reason: "currency_mismatch", jti: p.jti }, req);
                    throw createError("TXSIGN_INVALID", 403, "currency_mismatch");
                }
                if (payee && p.payee !== payee) {
                    audit.log("TX_INVALID", { reason: "payee_mismatch", jti: p.jti }, req);
                    throw createError("TXSIGN_INVALID", 403, "payee_mismatch");
                }
            }
