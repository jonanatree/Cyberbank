...
export async function registerDpopRoutes(app) {
    app.post("/dpop/verify", async (req) => {
        const proof = req.headers?.dpop || req.body?.proof;
        const { htm = req.body?.htm || "POST", htu = req.body?.htu } = req.body || {};
        if (!proof || !htu)
            throw createError("INVALID_REQUEST", 400, "missing_dpop_or_htu");
        try {
